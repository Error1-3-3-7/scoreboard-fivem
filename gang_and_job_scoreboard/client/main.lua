...
Citizen.CreateThread(function()
	...
	SendNUIMessage({
		...
		maxPlayers = GetConvarInt('sv_maxclients', 32), -- if you have more than "32" Players then change it to the count
		uptime = 'unknown',                   --   ^^
		playTime = '00h 00m'
	})
end)
...
function UpdatePlayerTable(connectedPlayers)
	...
	SendNUIMessage({
		...
		jobs   = {job_name = job_name, player_count = players}
	})
end
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(0, Keys['F10']) and IsInputDisabled(0) then
			ToggleScoreBoard()        -- ^^^^ This is the keybind that you open the scoreboard, and close it.
			Citizen.Wait(200)
